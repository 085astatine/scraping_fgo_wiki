...
def _parse_resource(text: str) -> list[Resource]:
    ...
    while match:
        ...
        _logger.debug("resource %s x %d", resource["name"], resource["piece"])
...
def _parse_ascension_resources(root: lxml.html.HtmlElement) -> list[ResourceSet]:
    parser = _ResourceSetParser(mode=_ResourceSetParserMode.ASCENSION)
    xpath = (
        '//div[@id="wikibody"]'
...
def _parse_skill_resources(
    ...
    parser = _ResourceSetParser(mode=_ResourceSetParserMode.SKILL)
    if target == "skill":
        xpath = (
            '//div[@id="wikibody"]'
...
def _load_servant(
    ...
    *,
    ...
    if path is not None and not force_update:
        result = load_json(path)
        if result is not None:
            _logger.debug('servant "%s" is loaded from "%s"', data["name"], path)
            # check
    ...
    for row in servant_table:
        ...
        result.append(
            _load_servant(
                ...
                costumes,
                path=(
                    directory.joinpath(f'{row["id"]:03d}.json')
...
def servant_dict() -> dict[str, Text]:
    ...
    for row in reversed(root.xpath(xpath)):
        ...
        _logger.debug('servant %03d: jp="%s", en="%s"', servant_id, name_jp, name_en)
